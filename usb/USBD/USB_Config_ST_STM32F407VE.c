...
Purpose : Config file for ST STM32F429I-Discovery.
--------  END-OF-HEADER  ---------------------------------------------
*/
...
void USBD_X_Config(void) {
  ...
  RCC_AHB1ENR |= 0
              | (1 <<  0)  // GPIOBEN: IO port B clock enable
              ;
  //
  // PA12,PA11 (OTG_HS_ internal alternate function, OTG_HS_DP, OTG_HS_DM, OTG_HS_ID)
  // 
  //
  GPIOA_MODER    =   (GPIOA_MODER  & ~(0x0FUL << 22)) | (0x0AUL << 24);  //AF для PA12, PA11 (первые биты в 1)
  GPIOA_OTYPER  &=  ~(0x03UL << 11);                                     // Push - pull
  GPIOA_OSPEEDR |=   (0x0FUL << 22);                                     //Very High speed
  GPIOA_PUPDR   &=  ~(0x0FUL << 22);                                     //без подтяжек
  GPIOA_AFRH     =   (GPIOA_AFRH  & ~(0xFFUL << 12)) | (0xAAUL << 12);   //AF10 для PA12, PA11
  //
  // STM32(Forum)
  // https://goo.gl/yh4wZ8
  //
